...
[data](https://www.npmjs.com/package/@aave/data).

```sh
// with npm
npm install @aave/contract-helpers @aave/data
// with yarn
yarn add @aave/contract-helpers @aave/data
```
...
2.  [Transaction Methods](#transaction-methods)
    ...
    - b. [Lending Pool V3](#lending-pool-v3)
      - [supply](#supply)
...
`@aave/data` package is a collection of methods to take raw input data from
these contracts, and format to use on a frontend interface such as
...
samples below: - [ethers.js](#ethers.js) - [Subgraph](#subgraph) -
[Caching Server](#caching-server)
...
The RPC URL is an endpoint which will execute your requests to the blockchain.
The
...
const incentiveDataProviderContract = new UiIncentiveDataProvider({
  uiIncentiveDataProviderAddress: uiIncentiveDataProviderAddress,
  provider: provider,
});
...
- @param `marketReferencePriceInUsd`
- @param `marketReferenceCurrencyDecimals`

```ts
import { formatReserves } from '@aave/data';
...
- @param `marketReferencePriceInUsd`
- @param `marketReferenceCurrencyDecimals`
- @param `reserveIncentives`

```ts
import { formatReservesAndIncentives } from '@aave/data';
...
- @param `marketReferenceCurrencyDecimals`
- @param `userReserves`
- @param `userEmodeCategoryId`

```ts
import { formatUserSummary } from '@aave/data';
