...
describe('ray math', () => {
	...
	describe('rayMul', () => {
		...
		it('not return decimal places', () => {
			expect(
				rayMul(new BigNumber(0.1).pow(30), RAY).decimalPlaces(),
			).toEqual(0)
		})
	...
	describe('rayPow and binomialApproximatedRayPow', () => {
		...
		`(
			...
			({ exponents, errorLte }) => {
				...
				expect(
					Math.abs(Number.parseFloat(diffPercentage)),
				).toBeLessThanOrEqual(errorLte)
			},
		)
		...
		it('should increase values over time', () => {
			...
			const ts1 = 2
			const ts2 = 3
			const accurateInterest1 = legacyCalculateCompoundedInterest(
				rate,
				ts1,
				0,
			)
			const approximatedInterest1 = calculateCompoundedInterest(rate, ts1, 0)
			const accurateInterest2 = legacyCalculateCompoundedInterest(
				rate,
				ts2,
				0,
			)
